...
def party_code_generator(party):
    return "{}{}".format(settings.PARTY_CODE_PREFIX, "{:03}".format(party.id-56))
...
def create_party_wallet(sender, instance, *args, **kwargs):
    if instance.is_wallet_party and not instance.wallet_set.filter(is_active=True).exists():
        Wallet.objects.create(party=instance, deduct_type="FD")
...
class Wallet(models.Model):
    ...
    upper_limit = models.DecimalField(max_digits=9, decimal_places=2, default=Decimal(20000.00))
